...
HEADER_LINES = [
  ...
  "# exists before the first public commit.",
...
def collect_contributor_rows(repo_root: Path) -> list[tuple[str, str]]:
  ...
  if result.returncode != 0:
    folded_error = result.stderr.casefold()
    if (
      "does not have any commits yet" in folded_error
      or "bad revision" in folded_error
      or "unknown revision" in folded_error
      or "ambiguous argument 'head'" in folded_error
    ):
      return []
...
def collect_author_seed_rows(repo_root: Path) -> list[tuple[str, str]]:
  authors_path = repo_root / "AUTHORS"
  if not authors_path.exists():
    return []

  rows: list[tuple[str, str]] = []
  for line in authors_path.read_text(encoding="utf-8").splitlines():
    stripped = line.strip()
    ...
    details = rest.split(")", maxsplit=1)[0]
    email = details.split("-", maxsplit=1)[0].strip()
    rows.append((name.strip(), email))
  return rows
...
def expected_contents(repo_root: Path) -> str:
  rows = collect_contributor_rows(repo_root)
  if not rows:
    rows = collect_author_seed_rows(repo_root)
  return render_contributors(normalize_contributors(rows))
